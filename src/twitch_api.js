...
const twitch = async () => {
  ...
  const headers = { 'Client-ID': process.env.CLIENT_ID, 'Authorization': `Bearer ${token.access_token}` };
  ...
  let isLive = await axios.get(twitchApiUrl, { headers: headers }).catch(err => console.log(err))
